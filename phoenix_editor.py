...
main_application.title('PHOENIX TEXT EDITOR')
...
background = tk.Menu(main_menu, tearoff=False)
  
theme_choice = tk.StringVar()
background_icons = (light_default_icon, light_plus_icon, dark_icon, red_icon, monokai_icon, night_blue_icon)
...
main_menu.add_cascade(label='Background Theme', menu=background)
...
scroll_bar_y = tk.Scrollbar(main_application)
scroll_bar_y.pack(side=tk.RIGHT, fill=tk.Y)
scroll_bar_y.config(command=text_editor.yview)
text_editor.config(yscrollcommand=scroll_bar_y.set)
...
def change_color():
    color_v = tk.colorchooser.askcolor()
    text_editor.configure(fg=color_v[1])
        ...
        if text_changed:
            ...
            if mbox is True:
                if url:
                    ...
                    conent2 = str(text_editor.get(1.0, tk.END))        
                    url = filedialog.asksaveasfile(mode='w', defaultextension='.txt', filetypes=(('Text file', '*.txt'), ('All files', '*.*')))
                    url.write(conent2)
                    url.close()
                    main_application.destroy()
...
edit.add_command(label='Clear_All', image=clear_icon, compound=tk.LEFT, accelerator='Ctrl+Alt+X', command=lambda:text_editor.delete(1.0, tk.END))
...
view.add_checkbutton(label='StatusBar', onvalue=1,offvalue=False,variable=show_statusbar,image=statusbar_icon,compound=tk.LEFT,command=hide_statusbar)
...
COUNT = 0
for i in color_dict:
    background.add_radiobutton(label=i, image=background_icons[COUNT], variable=theme_choice, compound=tk.LEFT, command=change_theme)
    COUNT += 1 

# ----------&&&&&&&&&& EXIT MAIN MENU FUNCTIONALITY &&&&&&&&&&--------------------------
